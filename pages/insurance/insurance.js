...
Page({
  ...
  close_tips(){
	...
	onShow() {
		const self = this;
		console.log('self.data.insuresData',self.data.insuresData)
	...
	chooseBx(e) {
		...
		if(self.data.insuresData.peopleIds.length>0){
			...
			console.log(self.data.insuresData)
	...
	getOrderDetail(e) {
		...
		const callback = (res) => {
			if(res.content){
				...
				self.data.totalNum = self.data.orderDetailData.totalNum[0]+self.data.orderDetailData.totalNum[1]+self.data.orderDetailData.totalNum[2]+self.data.orderDetailData.totalNum[3]
			};
			self.setData({
	...
	getInsuranceData() {
		...
		const callback = (res) => {
			...
			console.log('getMainData', self.data.insuranceData)
		};
